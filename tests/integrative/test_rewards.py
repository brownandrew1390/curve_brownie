...
from brownie import *
from helpers_test.utils import calc_cur_rewards, load_contract, tripool_to_meta

@given(index=strategy("uint8", min_value=0, max_value=38))

def test_current_strategy_best(
    index, alice, registry, tripool_rewards
):
    # Assumptions
    ...
    # Skip if the new strategy is invalid
    target = load_contract(registry.pool_list(index))
    new_rewards = tripool_to_meta(
        target,
        alice,
        tripool_rewards)
    ...
    chain.mine(timestamp=final_time)
    new_rewards = calc_cur_rewards(
        alice,
        [tripool_rewards, new_rewards])

    # Assert that the initial_strategy is better than new
